...
This compound has a crystal structure belonging to space group of $R\bar{3}$, and displays two magnetically ordered phases at low temperatures \cite{maignan_spin-induced_2020}.
...
The details of the magnetic structure will be reported elsewhere.
...
The AFP-NMR system consists of a radio-frequency (RF) coil for flipping the \He polarization and an NMR pickup coil located at the bottom of the \He cell for monitoring the \He polarization.
...
Thus, we can impose the conditions of $I^{++} = I^{--}$ and $I^{+-} = I^{-+}$, and can simplify the equations regarding the effect of imperfect polarization described in Appendix A.
...
\begin{figure}
    ...
    \caption{
        ...
        The red line represents the fitting result for  the NMR signal using an exponential decay function.
...
\begin{figure}
    ...
    \caption{
    (a) Raw data, obtained by integrating the intensity over the peak region on the 2D detector while scanning the sample rotation angle $\omega$. The red and blue symbols represent the non-spin-flip intensity $\mathcal{I}^{++}_\mathrm{obs}$ and spin-flip intensity $\mathcal{I}^{+-}_\mathrm{obs}$, respectively.
    (b) Corrected data along the $(H,0,2)$ line, obtained after converting the raw data into reciprocal-space coordinates. The red and blue symbols represent the corrected non-spin-flip and spin-flip scattering intensities, $I^{++}(H,K,L)$ and $I^{+-}(H,K,L)$, respectively.
    }
...
According to the literature, $\sigma(E_n)$ at $E_n = 14.7\ \textrm{meV}$ is $6992~\textrm{barn}$ \cite{passell_measurement_1966}.
...
Immediately after the \He cell was moved from the SEOP system to the beamline, we obtained the integrated intensities of the reflection with the \He polarization parallel and antiparallel to the incident neutron polarization.
These are referred to as $\mathcal{I}^\mathrm{++}_\mathrm{obs}(t=0)$ and $\mathcal{I}^\mathrm{+-}_\mathrm{obs}(t=0)$, respectively, following the same procedure as the measurement with the unpolarized \He cell.
...
From Eqs.~\ref{eq:int_obs_pp}, \ref{eq:int_obs_pm}, and \ref{eq:transmission_3He}, the \He polarization $P_\mathrm{He}(t)$ at $t=0$ ($P_\mathrm{He,0}$) satisfies the following relationship:
\begin{align}
...
Figure~\ref{fig:pol_trans_decay} shows the time dependence of the NMR signal of the Shimpachi cell, which agrees well with Eq.~\ref{eq:pol_3He} shown by the red solid curve.
...
The relaxation times $\tau$ for the Shimpachi and Kenji cells were determined to be $\tau = 131.1 \pm 0.7$ and $\tau = 128.4 \pm 0.3$ hours, respectively.
...
From Eqs.~\ref{eq:int_obs_pp} and \ref{eq:int_obs_pm}, the asymmetry between $\mathcal{I}^{++}_\mathrm{obs}$ and $\mathcal{I}^{+-}_\mathrm{obs}$ after subtracting the background $BG(x_i, y_j)$ from each, is given by
\begin{equation}
...
% Fig.6 (a)の説明→(b)の説明。
Figure~\ref{fig:polcorrection}(a) shows the box-integrated intensities as functions of $\omega$, without the polarization correction, when the \He polarization was parallel and antiparallel to the neutron polarization.
...
Figure~\ref{fig:polcorrection}(b) shows the profiles of the corrected non-spin flip and spin flip intensities, $\mathcal{I}^{++}$ and $\mathcal{I}^{+-}$, along the $(H,0,2)$ line, which were sliced from the three dimensional data.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figure/Figure6_v7.pdf}
    \caption{
...
(f), (g) Line profiles of $I^{++}$ and $I^{+-}$ through \(\boldsymbol{q}_3\) (f) and \(\boldsymbol{q}_1\) (g).
}
...
A more detailed study, including the low-temperature phase, will be published elsewhere [Ref. Osawa].
...
By combining the data measured in other regions of reciprocal space, the magnetic structure was determined to be an oblique sinusoidal magnetic structure [Ref. Osawa]. %
...
We thank Mr. Ohkawara (Tohoku University) for his kind support in operating the SEOP laser system in the 5G beamport experimental area.
...
With these conditions, $I^{++} = I^{--}$ and $I^{+-} = I^{-+}$ hold, and allowing Eq.~\ref{eq:Ipp} to be simplified into the following matrix form in terms of the non-spin flip and spin flip intensities.
\begin{equation}
    \begin{bmatrix}
        I^{++}_\mathrm{obs} \\
        I^{+-}_\mathrm{obs}
...
Since the \He polarization of the \textit{ex-situ} \Hensf~decays over time, both the neutron polarization efficiency $p_a$ and the neutron transmission $T_a$ become time-dependent.
...
In contrast, $T_p$ does not depend on time, as described in Sec.~\ref{sec:datareduction}.
It was treated as a single constant scale factor and set equal to 1.
In practice, this time-dependent correction was applied to each detector pixel.
As a result, data collected throughout the entire measurement period could be used for the polarization analysis.
