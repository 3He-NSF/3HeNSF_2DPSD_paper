...
This capability arises from the magnetic moment of the neutron, which directly interacts with magnetic moments of electrons in the material through the magnetic dipole-dipole interaction.
...
Both the \Hensf~coil for polarization analysis shown in Fig.~\ref{fig:Setup} and that for the SEOP system were equipped with an AFP NMR system \cite{ino_compact_2012}.
...
The \Hensf~was operated in an \textit{ex-situ} configuration, in which the \He cell was polarized in the SEOP system and then transported to the beamline for neutron measurements.
A darkroom housing the SEOP laser system was constructed in the 5G beamport experimental area.
After the \He polarization reached saturation, the \He cell was transported to the \Hensf~coil on the beamline.
Neutron measurements were performed while the \He polarization relaxed, resulting in decreases in both the neutron polarization and transmission.
Since the \He polarization decreased with time, two cells were used alternately.
While one cell was used for polarized-neutron measurements, the other was polarized in the SEOP system.
...
At each sample rotation angle $\omega=\omega_n$, we measured a pair of 2D intensity maps corresponding to non-spin-flip and spin-flip scattering by flipping the \He polarization.
We denote the observed intensities measured with the \He nuclear polarization parallel and antiparallel to the neutron polarization produced by the Heusler polarizer as $I^{++}_\mathrm{obs}$ and $I^{+-}_\mathrm{obs}$, respectively.
All intensity maps were normalized by the measurement time.
The measurements of $I^{++}_\mathrm{obs}$ and $I^{+-}_\mathrm{obs}$ were repeated while varying the sample rotation angle $\omega$.

To account for the time dependence of the \textit{ex-situ} \Hensf, the neutron polarization and transmission during each measurement were approximated by their values at the midpoint of the measurement.
...
In a full polarization analysis experiment, one would measure four scattering intensities, $I^{++}$, $I^{+-}$, $I^{-+}$, and $I^{--}$, corresponding to the four possible combinations of the polarizer and analyzer spin states~\cite{babcock_discussion_2017}.
...
Using these relations and the midpoint times defined above, the observed intensities at detector pixel $(x_i,y_j)$ are expressed as
\begin{widetext}
    \begin{align}
...
Instead, the parallel and antiparallel orientations of the \He polarization relative to the incident neutron polarization are represented by the signs in the factors $1\pm P_pP_a(t)$ in Eqs.~\ref{eq:int_obs_pp} and \ref{eq:int_obs_pm}.
...
By solving Eqs.~\ref{eq:int_obs_pp} and \ref{eq:int_obs_pm} for $I^{++}$ and $I^{+-}$, we obtained the equations of the polarization correction for the dataset measured at $\omega=\omega_n$ as follows:
\begin{widetext}
    \begin{align}
        I^{++}(\omega_n,x_i,y_j) =&
        \frac{1+P_pP_a(t_{2n})}
...
The asymmetry of the line profiles, defined as $(I^{++}-I^{+-})/(I^{++}+I^{+-})$, was calculated from the integrated intensities, and the results are summarized in Table~\ref{tab:asymmetry}.

We compared the measured asymmetries with those calculated for different magnetic moment directions.
Since the neutron polarization is parallel to the $z$ axis defined in Fig.~\ref{fig:measured_Qregion}(a), the spin-flip scattering arises from the Fourier-transformed magnetization components in the $xy$ plane, which contains the $a$ and $c$ axes.
...
These results demonstrate that the combination of a \Hensf~and a 2D detector enables polarization analysis of magnetic reflections distributed in three-dimensional reciprocal space.
\begin{table}[t]
  \centering
