...
Scattered neutrons were detected using a scintillation-type 2D detector with an active area of \(256 \times 256\) mm$^2$, employing wavelength-shifting-fiber readout~\cite{nakamura_large-area_2012}.
...
At each $\omega$, we measured the 2D intensity maps for the non-spin-flip ($I^{++}_\mathrm{obs}$) and spin-flip ($I^{+-}_\mathrm{obs}$) configurations by flipping the \He polarization.
The observed scattering intensity is given by the following equation:
\begin{widetext}
\begin{align}
    I^{++}_\mathrm{obs}(\omega,t_1) &= \frac{N_0T_pT_a(t_1)}{2}\left[\left(1+ P_pP_a(t_1)\right)\Sigma^{++}(\omega)+\left(1- P_pP_a(t_1)\right)\Sigma^{+-}(\omega)\right],
    \label{eq:int_obs_pp}\\
    I^{+-}_\mathrm{obs}(\omega,t_2) &= \frac{N_0T_pT_a(t_2)}{2}\left[\left(1- P_pP_a(t_2)\right)\Sigma^{++}(\omega)+\left(1+ P_pP_a(t_2)\right)\Sigma^{+-}(\omega)\right],
    \label{eq:int_obs_pm}
\end{align}
\end{widetext}
where $N_0$ is the number of incident neutrons from the reactor source.
$\Sigma^{+\pm}$ represents the scattering cross section of the reflection of interest.
$\Sigma^{++}$ and $\Sigma^{+-}$ correspond to the non-spin-flip and spin-flip scattering cross sections, respectively.
...
$t_1$ and $t_2$ are the central measurement times for $I^{++}_\mathrm{obs}$ and $I^{+-}_\mathrm{obs}$, respectively.
...
Consequently, the observed intensities $I^{++}_\mathrm{obs}(\omega,t_1)$ and $I^{+-}_\mathrm{obs}(\omega,t_2)$ in Eqs.~\ref{eq:int_obs_pp} and \ref{eq:int_obs_pm} are affected differently by the time-dependent imperfect neutron polarization and transmission.
The corrected intensities are obtained by solving Eqs.~\ref{eq:int_obs_pp} and \ref{eq:int_obs_pm} for $\Sigma^{++}$ and $\Sigma^{+-}$, yielding
\begin{widetext}
\begin{align}
    I^{++}_\mathrm{corr}(\omega) &= \frac{\left(1+ P_pP_a(t_2)\right)}{T_a(t_1)P_p\left(P_a(t_1)+P_a(t_2)\right)}I^{++}_\mathrm{obs}(\omega,t_1)-\frac{\left(1- P_pP_a(t_1)\right)}{T_a(t_2)P_p\left(P_a(t_1)+P_a(t_2)\right)}I^{+-}_\mathrm{obs}(\omega,t_2),\\
    I^{+-}_\mathrm{corr}(\omega) &= \frac{-\left(1- P_pP_a(t_2)\right)}{T_a(t_1)P_p\left(P_a(t_1)+P_a(t_2)\right)}I^{++}_\mathrm{obs}(\omega,t_1)+\frac{\left(1+ P_pP_a(t_1)\right)}{T_a(t_2)P_p\left(P_a(t_1)+P_a(t_2)\right)}I^{+-}_\mathrm{obs}(\omega,t_2),
    \label{eq:int_corr}
\end{align}
\end{widetext}
where $I^{+\pm}_\mathrm{corr}(\omega) = N_0T_p\,\Sigma^{+\pm}(\omega)$.
Here, $N_0T_p$ is treated as a time-independent scale factor and is absorbed into the corrected intensities.
...
According to the literature, the $\sigma(\lambda)$ at $\lambda = 2.36\ [\textrm{\AA}]$ is $6992~[\textrm{barns}]$ \cite{passell_measurement_1966}.

First, the $\rho d$ of each cells was determined.
...
Using Eqs.~\ref{eq:int_obs_pp} and \ref{eq:transmission_3He}, the observed intensity of nuclear Bragg peak reflection (non-spin flip scattering, $\Sigma^{++}$) through the unpolarized \He cell can be written as
\begin{equation}
    I^\mathrm{unpol}_\mathrm{obs}(\omega) = \frac{N_0T_p}{2}T_\mathrm{cell}\exp(-\mathcal{O}(\lambda))\Sigma^{++}(\omega).
\end{equation}
...
After polarizing the \He by SEOP, the \He cell was transported to the beamline, and then the $I^{++}_\mathrm{obs}$ and $I^{+-}_\mathrm{obs}$ were measured.
...
\begin{align}
    \frac{I^{++}_\mathrm{obs}(0)+I^{+-}_\mathrm{obs}(0)}{I^\mathrm{unpol}_\mathrm{obs}} = 2\cosh(\mathcal{O}(\lambda)P_\mathrm{He,0}).
